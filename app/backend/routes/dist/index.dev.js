...
var populateCon = require('../Controller/populateCon');

var chatCon = require('../Controller/chatCon');

var contactCon = require('../Controller/contactCon');

var _require = require('../utils/userValidation'),
    userRegValidate = _require.userRegValidate,
    userLoginValidate = _require.userLoginValidate;

var _require2 = require('../utils/auth'),
    ensureAuthenticated = _require2.ensureAuthenticated; // Import mediasoup routes


var mediasoupRoutes = require('./mediasoupRoutes'); // Import call controller


var callCon = require('../Controller/callCon'); // auth
...
routes.post('/isLoggedinUser', userCon.isLoggedinUser); // password reset

routes.post('/forgot-password', userCon.forgotPassword);
routes.post('/reset-password', userCon.resetPassword); // friend/Contacts
...
routes.get('/get-all-contacts', ensureAuthenticated, userCon.getAllContacts); // chat
...
routes["delete"]('/bookmark/:message_id/:user_id', ensureAuthenticated, chatCon.del_bookmark); // call list

routes.get("/get-call-list", ensureAuthenticated, userCon.getCallList); // archive
...
routes.post('/conversation/mute', ensureAuthenticated, chatCon.mute_conv); // for theme 

routes.post("/update_theme", ensureAuthenticated, userCon.update_theme);
routes.get("/get_theme", ensureAuthenticated, userCon.get_theme); // system event

routes.post("/conversation/:id/:type", ensureAuthenticated, chatCon.remove_user_frm_room); // setting->privacy

routes.post("/updatePhotoVisibility", ensureAuthenticated, chatCon.updatePhotoVisibility); // setting->Intro

routes.post("/updatePersonalInfo", ensureAuthenticated, chatCon.updatePersonalInfo); // location tile

routes.get("/tiles/:z/:x/:y", chatCon.mapTile); // mark unread msg

routes.put("/conversation/markUnread", ensureAuthenticated, chatCon.markAsUnRead);
routes.get("/file/:filename", ensureAuthenticated, chatCon.checkImageAuthorized); // populate data

routes.get('/reg_bulk', populateCon.registerBulk);
routes.get('/conn_bulk', populateCon.populateConnections); // contact

routes.get('/get-filtered-user', ensureAuthenticated, userCon.getAllFilteredUser);
routes.get('/turn-credentials', userCon.getTurnCred); // Contact form routes
...
routes["delete"]('/contact/:id', ensureAuthenticated, contactCon.deleteContact); // =========== MEDIASOUP ROUTES =============
// Mount mediasoup routes under /mediasoup prefix

routes.use('/mediasoup', mediasoupRoutes); // =========== CALL ROUTES =============
// Get call details by ID

routes.get('/call/:callId', ensureAuthenticated, callCon.getCallDetails); // Get all calls for a user

routes.get('/call/user/:userId', ensureAuthenticated, callCon.getUserCalls); // Update call status
